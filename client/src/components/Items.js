...
function ItemDetails(props) {
    ...
    if (item) return (
        ...
        <Box display='flex' flexDirection='column' height={`calc(100vh - 20px)`}>
            ...
            <Box sx={{ p: 1 }}><Skeleton variant='rectangular' height={100} /></Box>
            <Box sx={{ p: 1 }}><Skeleton /></Box>
            <Box  sx={{ borderRadius: '10px', overflow: 'hidden', m: 1, flexGrow: 1 }}>
...
const lastUpdatedText = (date) => {
    ...
    if (minutes < 60) return `${minutes === 1 ? 'a minute' : minutes + ' minutes'} ago`
